...
$.fn.zmSlider = function(e){
    ...
    switch(type){
        ...
        case "lineClamp":
            ...
            break;
        case "scale":
             nowSize = parseInt(temporary.css("width"));
            break;
        case "fontSpace":
        ...
        case "picCutTime"://laozhang
            ...
            nowSize = parseInt(slideSpeedArgs.split('speed=')[1].split('&')[0])/1000;
            break;
        ...
        case "shapeMarginBorR"://laozhang
            if(iSelected.find('path').attr('d').indexOf('L')>0){//竖线
                nowSize=iSelected.css('margin-right')
            }else {
                nowSize=iSelected.css('margin-bottom')
            }
            break;
        ...
        case "wordsLineHeight":
            nowSize = parseInt(temporary.css("line-height"))-parseInt(temporary.css("font-size"));
            break;
        case "wordsLineNum":
    ...
    if(type == 'scale'){
        ...
        value.val(parseInt(nowSize));//初始化input框的值
    }
    if(type == 'width'){
        value.val(nowSize? nowSize:"40")
    }
    ...
    value.on("change",function(){
        var changedVal = parseInt(value.val()) || nowSize;
        ...
        switch(type) {
            ...
            case "marginBottom":
                ...
                if(iSelected.hasClass('zm-edit-components-button')){
                    console.log(parseInt(iSelected.css('height')),2*changedVal)
                    iSelected.css('line-height',parseInt(iSelected.css('height'))-2*changedVal+'px');
                }
                break;//liu
            case "hoverBorderTopWidth":
            case "hoverBorderBottomWidth":
            ...
            case "scale":
                ...
                temporary.css("width", nowSize);
                temporary.css("height", nowSize);
                temporary.css("line-height", nowSize);
                temporary.css("font-size", nowSize - 24);
                break;
            case "fontSpace":
                nowSize = changedVal;
                if (nowSize > maxSize) {
            ...
            case "borderWidth_nav"://ye 3.15改动过后  设置mian层的整体宽度 设置mian层的整体宽度复用其他组件
                ...
                if(/vertical/.test(iSelected[0].classList[4])){
                    ...
                    var li_height=parseInt(zmEditor.globalMethod.nav.data.arrLike[data_a]["li"]["sStyle"]["height"])-borderWidth*2;
            ...
            case "picCutTime"://oldZhang    2017-3-14 14:26:25
                ...
                iSelected.find(".zm-component-carousel-box")
                    .attr('data-slide-args', 'speed=' + ( nowSize * 1000) + '&autoScroll' + slideSpeedArgs.split('autoScroll')[1]);
                iSelected.find(".zm-component-carousel-box").boxSlider('option', 'speed', nowSize * 1000);
                break;
            case "picStayTime"://oldZhang    2017-3-14 14:26:25
                ...
                iSelected.find(".zm-component-carousel-box").boxSlider('option', 'timeout', nowSize * 1000);
                break;
            case "marginLength"://oldZhang    2017-3-14 14:26:25
                ...
                iSelected.closest(".zm-component-carousel-main").find(".zm-component-carousel-controlBtn.left").css('left', nowSize);
                iSelected.closest(".zm-component-carousel-main").find(".zm-component-carousel-controlBtn.right").css('right', nowSize);
                break;
            ...
            case "wordsLineHeight":
                nowSize = changedVal;
                var multNum  =parseInt(temporary.css('height'))/parseInt(temporary.css('lineHeight'));
                temporary.css({
                    "lineHeight":changedVal +parseInt(temporary.css("font-size")) + 'px',
                    "height":(changedVal +parseInt(temporary.css("font-size")))*multNum + 'px'
                });
    ...
    child.on("mousedown",function(e){
        ...
        $(document).mousemove(function(e){
            ...
            if(left >= 0 && left <= pWidth-eWidth){
                value.val(parseInt(left*sDiff/wDiff+minSize));
                child.css({"left": left});
                child_hover.css('width',left);
                var changedVal = parseInt(value.val());
                switch(type){
                    case "fontSize":
                    case "width":
                    case "height":
                    ...
                    case "lineClamp":
                        ...
                        break;
                    case "scale":
                       temporary.css("width",changedVal+"px");
                        temporary.css("height",changedVal+"px");
                        temporary.css("line-height",changedVal+"px");
                        temporary.css("font-size",changedVal-24+"px");
                        break;
                    case "fontSpace":
                        temporary.css({"letterSpacing": changedVal+"px","padding-left":changedVal+"px"});
                        break;
                    case "tbPadding":
                        iSelected.css({
